...
The interim crew instruction emitted by `bin/fm-brief.sh` follows the board tool rule: start the foreground poll, or the harness-native tracked background job, BEFORE writing any status line about the board, never with bare `&`, `nohup`, `disown`, or redirected fire-and-forget polling, then post a keyed `needs-decision` carrying the live board URL, stop at `session_ended`, and never report done while that board's session is still open.

A board nobody polls fails silently: the browser tells the captain "Your agent is not listening", every comment queues on the server, and no status line, error, or wake is ever produced.
`bin/fm-lavish-board-guard.sh` is the watcher-side detection for exactly that, and its header owns the complete contract - the four gates, the grace period, the `FM_LAVISH_BOARD_GRACE_SECS` bound, and the per-board deduplication record.
It reads only the status logs, the recorded endpoints, the read-only `bin/fm-procevent-lavish.sh sessions` listing, and the process table, and it publishes one ordinary durable `check` wake per unattended board.
It never arms, polls, opens, or ends a board, because the rule above still stands: firstmate answers such a wake by re-ringing the hosting worker, or by the guarded adoption path once that worker is proved dead.
