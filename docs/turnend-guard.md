...
A session lock held by a live harness outside the hook's ancestry is the other inert shape, and it is handled before the budget: that hook fires in a session the home refused its lock, which is read-only by contract (AGENTS.md section 3) and may not repair supervision, while the fail-open it would otherwise wait for needs a failure episode the inert auto-arm can never record.
The guard therefore steps aside for it with one `systemMessage` naming the lock-holding pid and spends no budget; a missing, malformed, dead, or self-owned lock changes nothing, and the ordinary block stands (`lock_held_by_other_live_session` in `bin/fm-turnend-guard.sh`).
