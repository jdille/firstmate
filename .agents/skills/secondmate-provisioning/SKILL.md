...
The same placement-specific launch and deferred bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/backend`, `config/herdr-presentation-spaces`, `config/startup-memory-budget`, `config/trace-context`, and the one shared captain-preference file `data/captain-shared.md`.
`config/trace-context` is session-scoped: it converges at the launch point, where the primary also hands the new process its frozen decision, and live convergence leaves an already-running home unchanged (`docs/trace-context.md`).
